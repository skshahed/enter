<?php
//Return a string truncated to a maximum number of characters. If the 
//string has been truncated, if will have $tail appended to the end.
...
function show_comments ($db, $article_id, $show_link = TRUE) {
	...
	if ($show_link) {
		...
		if (isset($_SESSION['user_id']) and $is_published) {
			echo ' - <a href="comment.php?article_id=' . $article_id . '">Add one</a>';
		}
		echo '</h3>';
	}
