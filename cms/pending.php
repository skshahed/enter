...
if (mysql_num_rows($result) == 0) {
	...
	while ($row = mysql_fetch_assoc($result)) {
		echo '<li><a href="review_article.php?article_id=' . $row['article_id'] . '">' . htmlspecialchars($row['title']) . '</a> (' . date('F j, Y' , $row['submit_date']) . ')</li>';
	}
	echo '</ul>';
}
...
if (mysql_num_rows($result) == 0) {
	...
	echo '<ul>';
	while ($row = mysql_fetch_assoc($result)) {
		echo '<li><a href="review_article.php?article_id=' . $row['article_id'] . '">' . htmlspecialchars($row['title']) . '</a> (' . date('F j, Y' , $row['publish_date']) . ')</li>';
	}
	echo '</ul>';
}
mysql_free_result($result);
