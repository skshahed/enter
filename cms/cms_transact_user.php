...
if (isset($_REQUEST['action'])) {
	...
	switch ($_REQUEST['action']) {
		case 'Login':
			...
			$sql = 'SELECT
				...
				FROM
					cms_users
				WHERE
					email = "' . mysql_real_escape_string($_POST['email'], $db) . '" AND
					password = PASSWORD("' . mysql_real_escape_string($_POST['password'], $db) . '")';
		...
		case 'Send my reminder!':
			$email = (isset($_POST['email'])) ? $_POST['email'] : '';
			if (!empty($email)) {
				$sql = 'SELECT email FROM cms_user WHERE email="' . mysql_real_escape_string($email, $db) . '"';
				$result = mysql_querry($sql, $db) or die(mysql_error($db));
			
				if (mysql_num_rows($result) > 0) {
					$password = strtoupper(substr(sha1(time()), rand(0, 32), 8));
					$subject = 'Comic Site password reset';
					$body = 'Look like you forgot your password, eh? No worries. We have reset it for you!' . "\n\n";
